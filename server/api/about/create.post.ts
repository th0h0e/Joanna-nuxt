import { aboutSchema } from '~~/server/schemas/pocketbase'
...
export default defineEventHandler(async event => {
  ...
  try {
    if (contentType.includes('multipart/form-data')) {
        ...
        for (const part of parts) {
          ...
          const pbFieldName =
            fieldName === 'aboutDescription'
              ? 'About_Description'
              : fieldName === 'clientListJson'
                ? 'Client_List_Json'
                : fieldName === 'contactEmail'
                  ? 'Contact_Email'
                  : fieldName === 'contactMessage'
                    ? 'Contact_Message'
                    : fieldName === 'expertiseDescription'
                      ? 'Expertise_Description'
                      : fieldName === 'expertiseTitle'
                        ? 'Expertise_Title'
                        : fieldName === 'isActive'
                          ? 'Is_Active'
                          : fieldName === 'portfolioTitle'
                            ? 'Portfolio_Title'
                            : fieldName === 'selectedClientsTitle'
      ...
      const body = await readValidatedBody(event, aboutSchema.parse)

      if (body.aboutDescription !== undefined)
        formData.append('About_Description', body.aboutDescription)
      if (body.clientListJson !== undefined)
        formData.append(
          'Client_List_Json',
      ...
      if (body.expertiseDescription !== undefined)
        formData.append('Expertise_Description', body.expertiseDescription)
      ...
      if (body.selectedClientsTitle !== undefined)
        formData.append('Selected_Clients_Title', body.selectedClientsTitle)
    }
